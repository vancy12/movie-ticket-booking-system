...
function selectSeat(no, sclass) {
    ...
    if (seatIndex === -1) {
        // Seat not selected, add to selectedSeats and toggle visual indication
        selectedSeats.push({ no: no, sclass: sclass });
        seatButton.classList.add('selected');
    } else {
        // Seat already selected, remove it and toggle off visual indication
        selectedSeats.splice(seatIndex, 1);
        seatButton.classList.remove('selected');
    }

    // Send updated selected seats list to the server to update price or other info
    $.ajax({
        type: 'POST',
        url: '/getPrice',
        data: {
            showID: showID,
            seatClass: sclass,
            selectedSeats: JSON.stringify(selectedSeats) // Send selected seats as JSON
        },
        success: function(response) {
            $('#price-and-confirm').html(response);
        }
...
function selectShowHall(screen){
	$.ajax({
			type: 'POST',
			url: '/insertShow',
			data: {
